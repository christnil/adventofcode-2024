...
const findCheetsForPosition = (x, y) => {
    ...
    for (let i = x - 2; i <= x + 2; i++) {
        for (let j = y - 2; j <= y + 2; j++) {
            if (canVisit(i, j) && manhattanDistance([x, y], [i, j]) <= 2) {
...
const countCheetsOver100 = cheetArray.filter(([k, v]) => k >= 100).reduce((acc, [_, v]) => acc + v, 0);

console.log('Part1: ', countCheetsOver100);
